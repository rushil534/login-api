...
public class ResetUsername extends JFrame implements ActionListener {
	...
	private void replaceUsername(String u, String p, String cu, File e, JLabel t) throws FileCreatedInterruptionException {
		try {
					...
					if(u.trim().equals(sArray[0]) && p.trim().equals(EncryptionHandler.decrypt(sArray[1]))) {
						if(v.usernameExists(cu, e) == Status.USERNAME_EXISTS) {
					        ...
					        try {
					            ...
					            String needsToBeReplaced = u.trim() + ", " + EncryptionHandler.encrypt(p.trim());
					            while (line != null) {
					            ...
					            writer.write(cu.trim() + ", " + EncryptionHandler.encrypt(p));
					            reader.close();
					            writer.close();
					        }
