...
public class ResetPassword extends JFrame implements ActionListener {
	...
	private void replacePassword(String u, String p, String cp, File e, JLabel t) throws FileCreatedInterruptionException {
		try {
				...
				while(in.hasNextLine()) {
					...
					if(u.trim().equals(sArray[0]) && p.trim().equals(EncryptionHandler.decrypt(sArray[1]))) {
				        ...
				        try {
				            ...
				            String needsToBeReplaced = u.trim() + ", " + EncryptionHandler.encrypt(p);
				            while (line != null) {
				            ...
				            writer.write(u.trim() + ", " + EncryptionHandler.encrypt(cp));
				            reader.close();
				            writer.close();
				        }
				        catch (Exception e1) {}
	...
	public void actionPerformed(ActionEvent e) {
		if(e.getActionCommand().equals("confirm")) {
			...
			if(cp.length() <= 5) {
				JOptionPane.showMessageDialog(null, "Your new password must be 5 characters or greater\nErr code: Status.FAILURE", "Error", JOptionPane.ERROR_MESSAGE);
				return;
			}
			
			if(EncryptionHandler.strContainsUnallowedChar(cp)) {
				JOptionPane.showMessageDialog(null, "Unallowed character in new password field\nAllowed characters: A-Z a-z 0-9\nErr code: Status.FAILURE", "Error", JOptionPane.ERROR_MESSAGE);
				return;
			}
